...
- パッケージ構成: `app/`配下、ページごとに独立したReact（Vite）アプリ（例: `app/line-link/`、`app/allowed-emails/`、`app/voice-practice/`）のみ。`ci.yml`の`packages`にそれぞれlint/test/build対象として登録する。それ以外のリポジトリ全体としてはfrontend/backend固定ディレクトリ構成を持たないため、`reusable-ci.yml`のfrontend-test/backend-testジョブは使わない。MkDocsのビルド検証は別ワークフロー（`.github/workflows/mkdocs.yml`）で行う
