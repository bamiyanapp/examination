...
- **データモデル**:
  - `examination-families`（パーティションキー: `slug`＝作成時にランダム生成する短縮ID）: 家族の登録簿。属性`createdBy`・`createdAt`のみで、表示名・シチュエーションは持たない（[Issue #305](https://github.com/bamiyanapp/examination/issues/305)で家族名とシチュエーションを`examination-family-profile`へ統合）
...
- **家族の新規作成（公開登録制）**:
  ...
  2. `/family-create/`（Reactアプリ`app/family-create/`。他アプリと異なりPWA化・`UserMenu`等の共通コンポーネントは持たない。理由は`src/App.jsx`のコメント参照）で**シチュエーション**（例:「小学校受験の面接」、家族名ではない。[Issue #305](https://github.com/bamiyanapp/examination/issues/305)で統合）を入力すると、`POST /_families`（`checkAuth.js`の`createFamily`）が未所属であることを確認した上で`examination-families`へ新規行（`slug`・`createdBy`・`createdAt`のみ）を作成し、作成者自身を`examination-allowed-emails`へ追加する。シチュエーションはこの時点では一意性チェックを行わない（複数家族で同じシチュエーションを選んでも問題ないため）
  3. 作成成功時、`checkAuth.js`が`bot-stack`の内部API（`POST /internal/notify-family-created`、`functions/notifyFamilyCreated.js`）を呼ぶ。このAPIはまず入力されたシチュエーションを`examination-family-profile`へ種として保存し（以降`/settings/profile-edit/`でいつでも変更できる）、続けてサイト運営者（管理用Googleアカウント、`ADMIN_NOTIFY_EMAIL`）へLINEで通知する（[Issue #259](https://github.com/bamiyanapp/examination/issues/259)）。site-stackとbot-stackは別Serverless serviceのため、共有シークレット（`INTERNAL_API_SECRET`、`X-Internal-Secret`ヘッダー）で呼び出し元を検証する。管理用アカウントがLINE未連携の場合は通知をスキップするのみで、家族作成自体は失敗させない（ベストエフォート）
- **メンバー管理**: 上記「閲覧許可メールアドレスの管理」の通り、一覧・追加・削除は自分の所属家族の範囲に限定される
- **退会と家族データの完全削除**（[Issue #284](https://github.com/bamiyanapp/examination/issues/284)）: `/_admin/emails`で自分自身のメールアドレスを削除しようとした場合、家族に他のメンバーが残っていれば拒否する。自分が最後の1人の場合のみ許可し、退会と同時に家族の全データを削除する。`checkAuth.js`が`bot-stack`の内部API（`POST /internal/delete-family-data`、`functions/deleteFamilyData.js`。`notifyFamilyCreated.js`と同じ共有シークレット方式）を呼び、`examination-family-profile`・`examination-interview-questions`・`examination-mock-interviews`（いずれも対象`familySlug`の全件）・`examination-line-links`（対象emailの行）を削除させた上で、成功を確認してから`examination-allowed-emails`の自分の行と`examination-families`の家族レコード自体を削除する。内部API呼び出しが失敗した場合はsite-stack側のデータを削除せず、部分的な削除状態を避ける。`app/allowed-emails/`は削除前に`window.confirm`で警告を表示し、削除成功後はセッションが無意味になるため`/_logout`（Googleセッションも含む完全ログアウト）へ遷移する
- **初期データ**: `cd.yml`の「Seed families table and backfill familySlug」ステップが、`examination-families`が空の場合のみ初期家族（`chofu-suzuki`）を投入し、`familySlug`属性が未設定の既存の許可メールアドレスへ同じslugをバックフィルする（冪等）
- 一部の一度きりの移行スクリプト（`scripts/seed-interview-questions.js`・`scripts/seed-mock-interviews.js`・`scripts/seed-mock-interview-2026-08-23.js`）は、旧Markdownファイルまたは外部（塾）で実施した面接特訓講座に由来する初期家族（`chofu-suzuki`）固有のデータを移行するものであり、意図的に`chofu-suzuki`をハードコードしたまま残している（他家族の作成・運用には影響しない）
